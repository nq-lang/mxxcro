...
def render_conflict_globe() -> None:
    ...
    # Do NOT use **_DARK_LAYOUT here: xaxis/yaxis keys crash geo-type figures
    fig.update_layout(
        paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor=C["panel"],
        font=dict(family="Space Mono, monospace", color=C["text_dim"], size=9),
...
def build_sector_bar_chart(sectors: List[Dict]) -> go.Figure:
    ...
    fig.update_layout(**_DARK_LAYOUT, height=340,
                      margin=dict(l=160, r=40, t=8, b=8),
                      xaxis=dict(gridcolor=C["border"], tickfont=dict(size=8)),
...
def build_earnings_surprise_chart(symbol: str) -> go.Figure:
    ...
    fig.update_layout(**_DARK_LAYOUT, height=220, showlegend=True,
...
@media (max-width:768px){
  ...
  .stTabs [data-baseweb="tab"]{font-size:9px!important;padding:7px 8px!important;letter-spacing:0.8px!important;}
  .tp{padding:5px 8px!important;}
  .phdr{font-size:8px!important;padding:5px 10px!important;}
  div[data-testid="column"]{padding:0 1px!important;}
}
@media (max-width:480px){
  .stTabs [data-baseweb="tab"]{font-size:8px!important;padding:5px 5px!important;letter-spacing:0px!important;}
...
def _render_tab_macro_v2_inner() -> None:
    # Inject mobile CSS on every render
    ...
    with left_col:
        ...
        sectors = fetch_sector_performance()
        if sectors:
            phdr("S&P Sector Rotation", "ALPHA VANTAGE · REAL-TIME", "gold")
    ...
    with right_col:
        ...
        # Global exchange cross-listing (Marketstack)
...
def _render_tab_stock_v2_inner() -> None:
    ...
    with chart_col:
        ...
        # Earnings surprise chart (Alpha Vantage)
        phdr(f"EPS Surprise History — {ticker}", "ALPHA VANTAGE · QUARTERLY", "gold")
        st.plotly_chart(build_earnings_surprise_chart(ticker), use_container_width=True,
                        config={"displayModeBar": False})
        st.markdown("<br>", unsafe_allow_html=True)
...
def _render_tab_valuation_v2_inner() -> None:
    ...
    with dcf_col:
        ...
        # Earnings surprise
        phdr(f"EPS Surprise — {vt}", "ALPHA VANTAGE", "gold")
        st.plotly_chart(build_earnings_surprise_chart(vt), use_container_width=True,
                        config={"displayModeBar": False})
        st.markdown("<br>", unsafe_allow_html=True)
