...
def inject_css() -> None:
    ...
    .term-logo{{
        ...
        color:{C['gold']};letter-spacing:3.5px;
...
def _render_tab_macro_v2_inner() -> None:
    ...
    with left_col:
        ...
        # OSINT stream
        render_osint_stream(state, max_items=18)
        st.markdown("<div style='height:4px'></div>", unsafe_allow_html=True)

        # Live geopolitical news tabs
        phdr("Geopolitical News", "LIVE FEED", "gold")
        t1, t2 = st.tabs(["🌍 World", "⚡ Flash"])
        with t1:
            render_news_feed(state.news_macro[:8], max_items=8)
        with t2:
            breaking = [n for n in state.news_all
                        if n.get("tier") in ("FLASH","PRIORITY")][:10]
            render_news_feed(breaking)
...
def _render_tab_valuation_v2_inner() -> None:
    ...
    # ── Column 1: Scorecard + Factor Breakdown ──────────────────────────────────
    with score_col:
        phdr(f"Valuation Scorecard — {profile.get('name', vt)}")
        render_val_cards(profile)
        st.markdown(
    ...
    with model_col:
        ...
        # Earnings surprise
        phdr(f"EPS Surprise History — {vt}", "ALPHA VANTAGE", "gold")
    ...
    # ── Full-width Alpha Picks section ───────────────────────────────────────────
