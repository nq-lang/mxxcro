...
def inject_css() -> None:
    ...
    html,body,[class*="css"]{{font-family:'Space Mono','Courier New',Courier,monospace!important;
    ...
    .stTabs [data-baseweb="tab-list"]{{background-color:{C['panel']}!important;
        border-bottom:1px solid {C['border']}!important;padding:0 8px!important;gap:0!important;}}
    .stTabs [data-baseweb="tab"]{{background:transparent!important;color:{C['muted']}!important;
        font-family:'Syne',monospace!important;font-weight:700!important;font-size:10px!important;
        letter-spacing:2px!important;padding:10px 20px!important;border:none!important;
        border-bottom:2px solid transparent!important;border-radius:0!important;}}
    .stTabs [aria-selected="true"]{{color:{C['gold']}!important;
    ...
    [data-testid="metric-container"]{{background:{C['panel']}!important;
        border:1px solid {C['border']}!important;border-radius:3px!important;padding:8px 10px!important;}}
    [data-testid="stMetricLabel"]>div{{font-size:8px!important;letter-spacing:1.5px!important;
        text-transform:uppercase!important;color:{C['muted']}!important;
        font-family:'Space Mono',monospace!important;}}
    [data-testid="stMetricValue"]{{font-family:'Syne',sans-serif!important;font-size:17px!important;
    ...
    .tp{{background:{C['panel']};border:1px solid {C['border']};border-radius:3px;
        padding:8px 12px;margin-bottom:2px;}}
    .phdr{{font-family:'Syne',sans-serif;font-weight:700;font-size:9px;
        letter-spacing:2px;text-transform:uppercase;color:{C['gold']};
        padding:6px 12px;border-bottom:1px solid {C['border']};
        background:{C['panel_alt']};display:flex;align-items:center;gap:8px;}}
    .phdr-badge{{font-size:7px;letter-spacing:1.5px;padding:2px 6px;
        border-radius:2px;font-weight:700;}}
    .kr{{display:flex;justify-content:space-between;padding:3px 0;
        border-bottom:1px solid {C['border']};font-size:9px;}}
    .kk{{color:{C['muted']};flex:1;}}
    .kv{{color:{C['text']};font-weight:700;text-align:right;flex:1;}}
    .term-hdr{{background:{C['panel']};border-bottom:1px solid {C['border']};
        padding:7px 16px;display:flex;align-items:center;gap:16px;}}
    .term-logo{{font-family:'Syne',sans-serif;font-size:14px;font-weight:800;
        color:{C['gold']};letter-spacing:3px;}}
    .vc{{display:flex;gap:10px;padding:5px 0;border-bottom:1px solid {C['border']};align-items:flex-start;}}
    .vi{{width:22px;height:22px;border-radius:2px;display:flex;align-items:center;
        justify-content:center;font-size:11px;font-weight:700;flex-shrink:0;margin-top:2px;}}
    .ipass{{background:rgba(46,125,82,0.2);color:{C['green_br']};border:1px solid {C['green']}44;}}
    .ifail{{background:rgba(211,47,47,0.2);color:{C['red']};border:1px solid {C['red']}44;}}
    .iwarn{{background:rgba(192,122,42,0.2);color:{C['orange_br']};border:1px solid {C['orange']}44;}}
    .clogo{{width:42px;height:42px;border-radius:6px;background:rgba(197,168,97,0.12);
        border:1px solid {C['gold']}44;display:flex;align-items:center;justify-content:center;
        font-family:'Syne',sans-serif;font-size:18px;font-weight:800;color:{C['gold']};flex-shrink:0;}}
    .ticker-strip{{background:#000;border-bottom:1px solid {C['border']};
        padding:5px 14px;font-size:10px;white-space:nowrap;overflow-x:auto;
    ...
    .news-item{{padding:6px 0;border-bottom:1px solid {C['border']};}}
    .news-src{{font-size:7.5px;letter-spacing:1.5px;text-transform:uppercase;
        padding:1px 5px;border-radius:2px;font-weight:700;}}
    .news-title{{font-size:10px;line-height:1.4;color:{C['text']};margin-top:3px;}}
    ...
    .crucix-outer{{background:{C['panel']};border-bottom:2px solid {C['gold_dim']};
        display:grid;grid-template-columns:160px 1fr;align-items:center;
        overflow:hidden;height:30px;position:relative;}}
    .crucix-label{{background:{C['gold_dim']};color:#000;font-family:'Syne',sans-serif;
    ...
    .crucix-scroll{{display:inline-flex;align-items:center;gap:0;white-space:nowrap;
    ...
    .crucix-item{{display:inline-flex;align-items:center;gap:7px;padding:0 20px;
        border-right:1px solid {C['border2']};height:100%;}}
    .crucix-src{{font-size:7.5px;letter-spacing:1.5px;padding:1px 5px;border-radius:2px;
...
def fetch_fred_series(series_id: str, limit: int = 24) -> List[Dict]:
    """Fetch FRED time-series data for Z-score anomaly detection."""
...
def fetch_polygon_snapshot(symbols: List[str]) -> Dict[str, Dict]:
    """
    Polygon.io snapshot — real-time price for a basket of US tickers.
    Free tier: delayed data, 5 req/min.
    """
    if not symbols:
        return {}
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for t in data:
                ...
                c     = day.get("c") or t.get("lastTrade", {}).get("p", 0) or 0
                pc    = prev.get("c") or c
...
def fetch_tradier_market_status() -> Dict:
    """Tradier market clock — is the NYSE currently open?"""
...
def aggregate_all_news() -> Dict[str, List[Dict]]:
    ...
    """
    # ── Live API sources ─────────────────────────────────────────────────────────
    newsapi_markets = fetch_newsapi("stock market finance economy", 20)
    newsapi_macro   = fetch_newsapi("federal reserve inflation gdp", 15)
    gnews_breaking  = fetch_gnews("economy markets finance", 10)
    gnews_geo       = fetch_gnews("geopolitics conflict war sanctions", 10)
    newsdata_econ   = fetch_newsdata("finance economy markets", "en", 10)
    worldnews_geo   = fetch_worldnews("geopolitics economy conflict military", 10)

    # ── RSS fallback sources ─────────────────────────────────────────────────────
    rss_crypto = _fetch_rss_sync(_CRYPTO_RSS, "crypto")
    rss_market = _fetch_rss_sync(_MARKET_RSS, "markets")
    rss_macro  = _fetch_rss_sync(_MACRO_RSS,  "macro")
...
def get_stock_overview(symbols: List[str]) -> List[Dict]:
    """Build stock overview table from Finnhub + Polygon."""
    result = []
    ...
    for sym in symbols:
        try:
            price  = 0.0
            change = 0.0
            if sym in poly and poly[sym].get("price"):
            ...
            result.append({
                ...
                "price":      price,
                "change_24h": change,
                "market_cap": float(prof.get("market_cap", 0) or 0) * 1e6,
            })
        except Exception:
...
def build_earnings_chart(profile: Dict) -> go.Figure:
    ...
    labels = [f"Q{i+1}" for i in range(len(eps))]
...
def build_crypto_heatmap(coins: List[Dict]) -> go.Figure:
    ...
    fig.update_layout(**_DARK_LAYOUT, height=320, margin=dict(l=0, r=0, t=4, b=0))
    return fig
...
def phdr(title: str, badge: str = "", badge_type: str = "gold") -> None:
    ...
    st.markdown(
        f'<div class="phdr">{title}{badge_html}</div>',
...
def render_ticker_bar(prices: Dict[str, Dict]) -> None:
    """Scrollable ticker tape with live prices."""
    if not prices:
        return
    items_html = ""
    for label, sym in TICKER_BAR_SYMBOLS:
...
def render_live_news_ticker(news_items: List[Dict]) -> None:
    """Crucix-style horizontally scrolling news ticker."""
    if not news_items:
        return
...
def render_econ_calendar(events: List[Dict]) -> None:
    ...
    html_out = (f'<div class="tp" style="padding:4px;max-height:280px;overflow-y:auto;">'
                ...
                + "".join(f'<td style="padding:3px 6px;color:{C["gold"]};font-size:7.5px;letter-spacing:1px;">{h}</td>'
                          for h in ["IMP", "CCY", "EVENT", "ACTUAL", "FCST", "PREV"])
                + "</tr>")
    for ev in events[:25]:
        ...
        html_out += (
            ...
            f'<td style="padding:3px 6px;color:{C["text"]};">{ev.get("event","")[:38]}</td>'
...
def render_conflict_globe() -> None:
    """Simple Plotly scatter_geo as a globe-like map for conflict hotspots."""
    ...
    colors = [f"rgba(211,47,47,{s})" for s in sevs]

    fig = go.Figure(go.Scattergeo(
        lat=lats, lon=lons,
        mode="markers+text",
        marker=dict(
            size=[8 + s * 18 for s in sevs],
            color=sevs,
            colorscale=[[0, C["green"]], [0.4, C["orange"]], [1.0, C["red"]]],
            cmin=0, cmax=1,
            line=dict(color=C["bg"], width=0.5),
            colorbar=dict(thickness=10, len=0.6, tickfont=dict(size=7, color=C["muted"])),
        ),
        text=names,
        textfont=dict(size=7, color=C["muted"]),
        textposition="top center",
        customdata=[[n, t, f"{s*100:.0f}%"] for n, t, s in zip(names, types, sevs)],
        hovertemplate="<b>%{customdata[0]}</b><br>%{customdata[1]}<br>Severity: %{customdata[2]}<extra></extra>",
    ))
    fig.update_geos(
        projection_type="natural earth",
        showcoastlines=True, coastlinecolor=C["border2"],
        showland=True, landcolor="#0d1117",
        showocean=True, oceancolor="#080a0f",
        showlakes=True, lakecolor="#080a0f",
        showframe=False,
        bgcolor=C["bg"],
    )
    fig.update_layout(
...
def build_zscore_bar_chart(macro: Dict) -> go.Figure:
    ...
    fig.update_layout(**_DARK_LAYOUT, height=260,
                      xaxis=dict(tickfont=dict(size=7), gridcolor=C["border"]),
                      yaxis=dict(gridcolor=C["border"], tickfont=dict(size=8),
...
def build_beta_chart(betas: Dict[str, float]) -> go.Figure:
    ...
    fig.update_layout(**_DARK_LAYOUT, height=230,
                      yaxis=dict(title=dict(text="Beta vs SPY", font=dict(size=8)),
...
def render_tab_macro_v2() -> None:
    ...
    with left_col:
        ...
        phdr("Infrastructure Cascade Risk", "CHOKEPOINTS · SUPPLY CHAIN", "red")
        ...
        # OSINT stream
        render_osint_stream(state, max_items=25)
        st.markdown("<br>", unsafe_allow_html=True)

        # FRED Z-Score anomaly chart
    ...
    with right_col:
        ...
        # Fear & Greed
        phdr("Fear & Greed Indices", "REAL-TIME", "gold")
        fg1, fg2 = st.columns(2)
        with fg1:
            render_fear_greed_gauge(state.crypto_fear_greed, "CRYPTO F&G")
        with fg2:
        ...
        with t2:
            render_news_feed([n for n in state.news_all
                              if n.get("asset_type") in ("macro","markets")][:10])
        with t3:
...
def render_tab_stock_v2() -> None:
    ...
    with h1:
        ticker_in = st.text_input("Ticker", value="AAPL", key="stock_ticker",
            label_visibility="collapsed", placeholder="Ticker (AAPL, TSLA, NVDA…)")
    with h2:
        days_map   = {"1M": 35, "3M": 95, "6M": 185, "1Y": 370, "2Y": 740}
    ...
    price = fq.get("price",  profile.get("current_price", 0))
    chg   = fq.get("change", 0)
    hi    = fq.get("high",   price)
    lo    = fq.get("low",    price)
    prev  = fq.get("prev",   price)
    chg_abs = price - prev
    ...
    # Data source badge
    src_badge = "POLYGON" if not fetch_polygon_ohlcv.func_doc or df is not None else "YAHOO"
    ...
    with chart_col:
        ...
        # Alpha Vantage macro beta chart
        phdr("Macro Beta vs S&P 500 (60-Day)", "ALPHA VANTAGE · DERIVED", "blue")
        av_prices = fetch_alphav_correlation_data(["SPY", ticker, "QQQ", "IWM"])
        if len(av_prices) >= 2:
            betas = compute_macro_beta(av_prices)
            if betas:
                st.plotly_chart(build_beta_chart(betas), use_container_width=True,
                                config={"displayModeBar": False})
    ...
    with info_col:
        ...
        relevant = [n for n in state.news_stocks
                    if ticker.lower() in n.get("title","").lower()][:4]
        if not relevant:
            relevant = state.news_stocks[:6]
        render_news_feed(relevant, max_items=6)
        st.markdown("<br>", unsafe_allow_html=True)
...
def render_tab_valuation_v2() -> None:
    ...
    with card_col:
        ...
        # Quantamental factor breakdown
        phdr("Factor Breakdown", "LIVE SCORING", "blue")
        detail    = qr.get("detail", {})
        fmax_map  = {"valuation":30,"financial_health":20,"technical":15,"macro_regime":20,"sentiment":15}
        for factor, max_val in fmax_map.items():
            ...
            st.markdown(
                ...
                f'<span style="color:{fc};">{sc:.0f}/{max_val}</span></div>'
                f'<div style="background:{C["border2"]};height:4px;border-radius:2px;">'
    ...
    with dcf_col:
        ...
        phdr("DCF Sensitivity — WACC × Terminal Growth", "HEATMAP", "gold")
        st.plotly_chart(
            build_dcf_sensitivity(profile["current_price"],
                                  profile.get("fair_value_dcf", 150) or 150),
            use_container_width=True, config={"displayModeBar": False})
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        # Marketstack cross-market data for selected ticker
        ...
        m_keys = ["Fed Funds Rate","10Y Treasury","2Y Treasury",
                  "US CPI YoY","US GDP (QoQ)","VIX Index","Yield Spread"]
        neg_up = {"Fed Funds Rate","10Y Treasury","2Y Treasury","US CPI YoY","Yield Spread"}
        mc_cols = st.columns(3)
        for i, key in enumerate(m_keys):
            info  = macro.get(key, MACRO_SEED.get(key, {}))
